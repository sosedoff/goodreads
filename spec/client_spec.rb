...
describe 'Client' do
  ...
  it 'returns friend details' do
  	@client.stub(:oauth_request).and_return(Hash.from_xml(fixture('friends.xml'))['GoodreadsResponse'])
    proc { @friends = @client.friends('878044') }.should_not raise_error
    @friends.should be_an_instance_of Hashie::Mash
    @friends.respond_to?(:user).should == true
    @friends.user.size.should == @friends.end.to_i
    @friends.user.first.respond_to?(:name)
  end
